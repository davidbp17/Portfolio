...
## Turtle Graphics with Mementos

This project demonstrates use of several important design patterns along with a parser to interpret commands. In this project the patterns that are implemented are the Visitor pattern and Memento Pattern. The turtle memento in this project holds state information and allows for undo operations, this can be demonstrated in tests. Visitor pattern is used in this example to calculate different distance metrics. There are expressions for all expressions such as pen up, pen down, move, turn and repeat, these are parsed through a text file.
...
[Link to the repository](./Turtle-Graphics-with-Mementos)
